...
class ExportService:
    ...
    def _build_ffmpeg_command(
        ...
        for text_index, text_clip in enumerate(text_clips):
            escaped_content = text_clip.content.replace("'", "'\\'").replace(":", "\\:").replace(";", "\\;")
            overlay_label = f"tov{text_index}"
            x_pos = int(text_clip.position_x * project.width)
            y_pos = int(text_clip.position_y * project.height)
            ...
            filter_parts.append(
                ...
                f"fontsize={text_clip.font_size}:"
                f"fontcolor={text_clip.color}:"
                f"x={x_pos}:y={y_pos}:"
                f"enable='between(t,{start_seconds:.6f},{end_seconds:.6f})'"
                f"[{overlay_label}]"
            )
